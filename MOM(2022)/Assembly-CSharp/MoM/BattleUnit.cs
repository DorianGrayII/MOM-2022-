...
namespace MOM
{
    ...
    public class BattleUnit : BaseUnit
    {
        ...
        public static BattleUnit Create(Unit source, bool abstractMode = false, int ownerOverrideForSummon = -1, bool attackingSide = false)
        {
        ...
        public Formation GetOrCreateFormation(IPlanePosition owner = null, bool createIfMissing = true)
        {
            ...
            if (this.battleFormation == null && createIfMissing)
            {
                ...
                {
                    bool flag = false;
                    foreach (KeyValuePair<BattleUnit, Unit> item in Battle.GetBattle().buToSource)
                    {
                        if (item.Key == this)
                        {
                            flag = true;
