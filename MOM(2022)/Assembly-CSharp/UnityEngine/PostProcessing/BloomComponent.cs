namespace UnityEngine.PostProcessing
{
    public sealed class BloomComponent : PostProcessingComponentRenderTexture<BloomModel>
    {
        ...
        public void Prepare(RenderTexture source, Material uberMaterial, Texture autoExposure)
        {
            ...
            Material material = base.context.materialFactory.Get("Hidden/Post FX/Bloom");
            material.shaderKeywords = null;
            material.SetTexture(Uniforms._AutoExposure, autoExposure);
            int width = base.context.width / 2;
            int num = base.context.height / 2;
            ...
            RenderTexture renderTexture = base.context.renderTextureFactory.Get(width, num, 0, format);
            Graphics.Blit(source, renderTexture, material, 0);
            RenderTexture renderTexture2 = renderTexture;
            for (int i = 0; i < num4; i++)
            {
                this.m_BlurBuffer1[i] = base.context.renderTextureFactory.Get(renderTexture2.width / 2, renderTexture2.height / 2, 0, format);
                int pass = ((i == 0) ? 1 : 2);
            ...
            {
                RenderTexture renderTexture3 = this.m_BlurBuffer1[num7];
                material.SetTexture(Uniforms._BaseTex, renderTexture3);
                this.m_BlurBuffer2[num7] = base.context.renderTextureFactory.Get(renderTexture3.width, renderTexture3.height, 0, format);
                Graphics.Blit(renderTexture2, this.m_BlurBuffer2[num7], material, 3);
                renderTexture2 = this.m_BlurBuffer2[num7];
            }
            RenderTexture renderTexture4 = renderTexture2;
            for (int j = 0; j < 16; j++)
            {
                if (this.m_BlurBuffer1[j] != null)
                {
                    base.context.renderTextureFactory.Release(this.m_BlurBuffer1[j]);
                }
                if (this.m_BlurBuffer2[j] != null && this.m_BlurBuffer2[j] != renderTexture4)
                {
                    base.context.renderTextureFactory.Release(this.m_BlurBuffer2[j]);
                }
                this.m_BlurBuffer1[j] = null;
                this.m_BlurBuffer2[j] = null;
            }
