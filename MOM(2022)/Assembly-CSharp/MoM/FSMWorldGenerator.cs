...
namespace MOM
{
    ...
    public class FSMWorldGenerator : FSMStateBase
    {
        ...
        private IEnumerator WorldBuilder()
        {
            ...
            new Vector2i(40, 26);
            int num = ((EntityManager.GetEntityOfType<Location>() != null) ? World.Get().worldSizeSetting : DifficultySettingsData.GetSettingAsInt("UI_WORLD_SIZE"));
            Vector2i size;
            switch (num)
            {
            case 0:
            ...
            if (EntityManager.GetEntityOfType<Location>() == null)
            {
        ...
        public void InitializeLocation(global::WorldCode.Plane p, List<Vector3i> connectingTowers)
        {
            ...
            for (int m = 0; m < connectingTowers.Count; m++)
            {
                ...
                Location.CreateLocation(connectingTowers[m], p, source, 0);
            }
            ...
            for (int num18 = 0; num18 < wizards.Count; num18++)
            {
                ...
                Group localGroup = townLocation.GetLocalGroup();
                ...
                else
                {
                    ...
                    {
                        localGroup.AddUnit(Unit.CreateFrom(item4));
                    }
                    localGroup.Position = position;
                    localGroup.GetMapFormation();
                    foreach (Reference<Unit> unit in localGroup.GetUnits())
                    {
            ...
            for (int num24 = 0; num24 < num20; num24++)
            {
                ...
                Location.CreateLocation(list3[num23], p, source, 0);
            ...
            for (int num29 = num23; num29 < num28; num29++)
            {
                if (num27 <= list3.Count / 2)
                {
                    ...
                    Location.CreateLocation(list3[num29], p, list6[index4], 0);
                    num27++;
                }
                else
                {
                    int index5 = global::UnityEngine.Random.Range(0, type3.Count);
                    Location.CreateLocation(list3[num29], p, type3[index5], 0);
                }
            }
        ...
        private void SpawnWaterLocations(global::WorldCode.Plane p, ELocationType lType, int lairReduction)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (!flag || GameManager.GetLocationsOfThePlane(p).Find((Location l) => l.GetDistanceTo(hex) <= 1) != null)
                {
                    continue;
                }
                ...
                if (location != null)
                {
                    Location.CreateLocation(hex, p, location, 0);
                    num--;
                    if (num <= 0)
                    {
                        break;
