...
public class JsonMapper
{
    ...
    private static object ReadValue(Type inst_type, JsonReader reader)
    {
        ...
        if (reader.Token == JsonToken.Double || reader.Token == JsonToken.Int || reader.Token == JsonToken.Long || reader.Token == JsonToken.String || reader.Token == JsonToken.Boolean)
        {
            ...
            {
                return JsonMapper.custom_importers_table[type2][type](reader.Value);
            }
            if (JsonMapper.base_importers_table.ContainsKey(type2) && JsonMapper.base_importers_table[type2].ContainsKey(type))
            {
                return JsonMapper.base_importers_table[type2][type](reader.Value);
            }
            if (type.IsEnum)
            {
    ...
    private static void WriteValue(object obj, JsonWriter writer, bool writer_is_private, int depth)
    {
        ...
        if (obj is Array)
        {
            ...
            {
                JsonMapper.WriteValue(item, writer, writer_is_private, depth + 1);
            }
        ...
        if (obj is IList)
        {
            ...
            {
                JsonMapper.WriteValue(item2, writer, writer_is_private, depth + 1);
            }
            writer.WriteArrayEnd();
            return;
        }
        if (obj is IDictionary)
        {
            ...
            foreach (DictionaryEntry item3 in (IDictionary)obj)
            {
                ...
                JsonMapper.WriteValue(item3.Value, writer, writer_is_private, depth + 1);
            }
        ...
        foreach (PropertyMetadata item4 in list)
        {
            if (item4.IsField)
            {
                ...
                JsonMapper.WriteValue(((FieldInfo)item4.Info).GetValue(obj), writer, writer_is_private, depth + 1);
                continue;
            }
    ...
    public static string ToJson(object obj)
    {
        lock (JsonMapper.static_writer_lock)
        {
            ...
            JsonMapper.WriteValue(obj, JsonMapper.static_writer, writer_is_private: true, 0);
    ...
    {
        JsonMapper.WriteValue(obj, writer, writer_is_private: false, 0);
    }
    ...
    {
        JsonReader reader2 = new JsonReader(reader);
        return (T)JsonMapper.ReadValue(typeof(T), reader2);
    }
