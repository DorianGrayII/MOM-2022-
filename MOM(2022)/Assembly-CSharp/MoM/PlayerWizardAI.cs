...
public class PlayerWizardAI : PlayerWizard
{
    ...
    public IEnumerator PlayTurn(int playerDoomStackValue)
    {
        ...
        if (base.heroes == null)
        {
            base.heroes = new List<Reference<global::MOM.Unit>>();
        }
        if (base.heroes.Count > 0)
        {
            if (base.artefacts != null && base.artefacts.Count > 0)
            {
                ...
                {
                    global::MOM.Artefact artefact = base.artefacts[num];
                    foreach (Reference<global::MOM.Unit> hero in base.heroes)
                    {
                        if (artefact == null)
                        {
                            break;
            ...
            foreach (Reference<global::MOM.Unit> hero2 in base.heroes)
            {
                int num2 = 0;
                ...
                global::MOM.Artefact artefact2 = global::MOM.Artefact.RandomFactory(num3, equipmentSlot.slotType.Get().eTypes, reportError: false);
                if (artefact2 != null)
                {
                    if (equipmentSlot.item != null)
                    {
        ...
        List<global::MOM.Group> list = new List<global::MOM.Group>(GameManager.GetGroupsOfWizard(base.ID));
        List<Multitype<global::MOM.Group, int>> list2 = new List<Multitype<global::MOM.Group, int>>();
        if (list.Count > 0)
        {
            for (int num4 = list.Count - 1; num4 >= 0; num4--)
            {
                global::MOM.Group group = list[num4];
                ...
                if (group.GetLocationHost() == null)
                {
                    ...
                    {
                        list2.Add(new Multitype<global::MOM.Group, int>(group, group.GetValue()));
                    }
                }
            }
        }
        list2.Sort((Multitype<global::MOM.Group, int> a, Multitype<global::MOM.Group, int> b) => -a.t1.CompareTo(b.t1));
        for (int j = 0; j < list2.Count; j++)
        {
            Multitype<global::MOM.Group, int> multitype = list2[j];
        ...
        List<global::MOM.Location> locations = GameManager.GetLocationsOfWizard(base.ID);
        ...
        this.PlanMelding();
        list = new List<global::MOM.Group>(GameManager.GetGroupsOfWizard(base.ID));
        foreach (global::MOM.Group v in list)
        {
            ...
            if (v.alive)
            {
                ...
                v.GetPosition();
                v.CurentMP();
                global::MOM.Group group2 = GameManager.GetGroupsOfPlane(v.GetPlane()).Find((global::MOM.Group o) => o != v && o.GetOwnerID() == v.GetOwnerID() && o.GetPosition() == v.GetPosition());
                if (group2 != null)
                {
        ...
        {
            global::MOM.Location location = this.FindNodeToMeld();
            if (location != null)
            {
                bool flag = false;
                foreach (global::MOM.Group item2 in GameManager.GetGroupsOfWizard(base.ID))
                {
                    List<Reference<global::MOM.Unit>> units = item2.GetUnits();
                    if (units.Count >= 1 && !(units.Find((Reference<global::MOM.Unit> o) => o.Get().IsMelder()) == null) && item2.aiDesignation != null)
                    {
                ...
                if (flag)
                {
                    global::MOM.Location location2 = null;
                    int num5 = int.MaxValue;
                    foreach (global::MOM.Location item3 in GameManager.GetLocationsOfWizard(base.ID))
                    {
    ...
    private void EnsureSpellTransport()
    {
        ...
        {
            if (!(entity.Value is global::MOM.Group group) || group.GetOwnerID() != base.ID || group.GetUnits().Count <= 4 || (!group.doomStack && this.priorityTargets?.GetAssignedTarget(group) == null) || group.waterMovement)
            {
                continue;
            }
    ...
    private IEnumerator PrepareLocations(List<global::MOM.Location> locations)
    {
        float realtimeSinceStartup = Time.realtimeSinceStartup;
        foreach (global::MOM.Location location in locations)
        {
    ...
    public void ResetFarmers(List<global::MOM.Location> locations)
    {
        foreach (global::MOM.Location location in locations)
        {
    ...
    public void PlanTaxes(List<global::MOM.Location> wizardLocations)
    {
        int count = DataBase.GetType<Tax>().Count;
        List<global::MOM.Location> list = wizardLocations.FindAll((global::MOM.Location o) => o is TownLocation);
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            int num3 = 0;
            int num4 = 0;
            foreach (global::MOM.Location item in list)
            {
    ...
    private void Oszustwa(List<global::MOM.Location> locations)
    {
        ...
        if (settingAsInt > 1 && base.wizardTower?.Get() != null && (TurnManager.GetTurnNumber() == 10 || (settingAsInt > 2 && TurnManager.GetTurnNumber() == 25) || (settingAsInt > 3 && TurnManager.GetTurnNumber() == 50)))
        {
            ...
            if (unit != null)
            {
                townLocation.AddUnit(global::MOM.Unit.CreateFrom(unit));
            }
        }
        int num3 = 0;
        int num4 = 0;
    ...
    private void PlanMelding()
    {
        ...
        foreach (global::MOM.Location visibleLocation in this.GetVisibleLocations())
        {
            ...
            foreach (global::MOM.Group ownGroup in this.GetOwnGroups())
            {
        ...
        foreach (global::MOM.Group ownGroup2 in this.GetOwnGroups())
        {
            if (ownGroup2.GetPlane() != location.GetPlane() || (ownGroup2.GetDesignation() != null && ownGroup2.GetDesignation().designation != AIGroupDesignation.Designation.Retreat) || !(ownGroup2.GetUnits().Find((Reference<global::MOM.Unit> o) => o.Get().IsMelder()) != null))
            {
                continue;
            }
            global::MOM.Unit unit = ownGroup2.KickOutMelder();
            if (unit == null)
            {
                continue;
            }
            global::MOM.Group group = unit.group?.Get();
        ...
        foreach (global::MOM.Location visibleLocation2 in this.GetVisibleLocations())
        {
        ...
        if (dBReference != null && base.banishedTurn == 0 && dBReference.Get().worldCost < base.mana)
        {
            ...
            unit2 = ((dBReference.Get() != (Spell)SPELL.GUARDIAN_SPIRIT) ? global::MOM.Unit.CreateFrom((global::DBDef.Unit)(Enum)UNIT.ARC_MAGIC_SPIRIT) : global::MOM.Unit.CreateFrom((global::DBDef.Unit)(Enum)UNIT.LIF_GUARDIAN_SPIRIT));
            location2.AddUnit(unit2);
        }
    }

    public void PlanFoodSupply(List<global::MOM.Location> locations)
    {
        float num = 0.2f;
    ...
    private IEnumerator ResolveLocationTactics(List<global::MOM.Location> locations)
    {
        foreach (global::MOM.Location location in locations)
        {
            ...
            {
                foreach (global::MOM.Location location2 in locations)
                {
        ...
        foreach (global::MOM.Location location3 in locations)
        {
    ...
    public int WaitTimeForTransporter(PlanePathStage stage, global::MOM.Group cargoGroup, bool bookTransport = false)
    {
        ...
        foreach (global::MOM.Group v in GameManager.GetGroupsOfPlane(plane))
        {
            ...
            {
                Reference<global::MOM.Unit> reference2 = null;
                bool flag2 = true;
                foreach (Reference<global::MOM.Unit> unit2 in v.GetUnits())
                {
        ...
        foreach (global::MOM.Location item in GameManager.GetLocationsOfThePlane(plane))
        {
    ...
    private void BookUnitTransportForCargo(global::MOM.Group sourceGroup, global::MOM.Unit source, global::MOM.Group cargo, Vector3i transportPos)
    {
        if (source == null)
        {
            return;
        }
        global::MOM.Group group = sourceGroup;
        if (sourceGroup.locationHost != null || sourceGroup.GetUnits().Count > 0)
        {
            group = new global::MOM.Group(sourceGroup.GetPlane(), sourceGroup.GetOwnerID());
    ...
    public AIWarEffort GetWareffortForGroup(global::MOM.Group g)
    {
    ...
    public static IEnumerator MoveGroup(global::MOM.Group g, Vector3i destination, bool arcanus)
    {
        yield return PlayerWizardAI.MoveGroup(g, destination, arcanus ? World.GetArcanus() : World.GetMyrror());
    }

    public static IEnumerator MoveGroup(global::MOM.Group g, Vector3i destination, global::WorldCode.Plane destinationPlane)
    {
    ...
    public global::MOM.Location FindNodeToMeld(global::MOM.Group closetoThisGroup = null)
    {
        global::MOM.Location location = null;
        foreach (global::MOM.Location visibleLocation in this.GetVisibleLocations())
        {
