...
namespace MOM
{
    public class PopupUnits : ScreenBase
    {
        ...
        public static void OpenPopup(ScreenBase parent, object units, string title = null, Callback cancel = null, Callback select = null)
        {
            ...
            if (units is List<Reference<Unit>>)
            {
                PopupUnits.instance.gridUnits.UpdateGrid(units as List<Reference<Unit>>);
            }
            else
            {
        ...
        {
            BaseUnit u = null;
            if (source is Reference<Unit>)
            {
                u = (source as Reference<Unit>).Get();
            }
                ...
                else
                {
                    ...
                    if ((bool)this.gridUnits)
                    {
                        foreach (object item2 in this.gridUnits.GetItems())
                        {
                            ...
                            if (item2 is Reference<Unit>)
                            {
                                list2.Add((item2 as Reference<Unit>).Get());
                            }
                            else
                            {
                                list2.Add(item2);
            ...
            component.gameObject.GetOrAddComponent<MouseClickEvent>().mouseRightClick = delegate
            {
                ...
                if ((bool)this.gridUnits)
                {
                    foreach (object item3 in this.gridUnits.GetItems())
                    {
                        ...
                        if (item3 is Reference<Unit>)
                        {
                            list.Add((item3 as Reference<Unit>).Get());
                        }
                        else
                        {
                            list.Add(item3);
