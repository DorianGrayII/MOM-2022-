...
namespace MOM
{
    public class TurnManager : MonoBehaviour
    {
        ...
        private IEnumerator EndTurnUpdate()
        {
            ...
            foreach (PlayerWizard v in GameManager.Get().wizards)
            {
                ...
                if (v.isAlive && v.banishedTurn > 0 && v.GetWizardStatus() == PlayerWizard.WizardStatus.Killed)
                {
                    ...
                    {
                        if (!(entity.Value is Location location) || location.GetOwnerID() != v.ID || location.GetUnits().Count <= 0)
                        {
                            continue;
                        }
                    ...
                    {
                        if (entity2.Value is Location location2 && location2.GetOwnerID() == v.ID)
                        {
                        ...
                        if (entity2.Value is Group group && group.GetOwnerID() == v.ID && !group.IsHosted())
                        {
                            list.Add(group);
                        }
                    }
        ...
        private void IncomeAndUpkeep(PlayerWizard v)
        {
            ...
            {
                Group group = GameManager.Get().registeredGroups[num7];
                if (group.GetOwnerID() == v.ID && (group.GetLocationHost()?.otherPlaneLocation?.Get() == null || !group.plane.arcanusType))
                {
                    List<Reference<Unit>> units = group.GetUnits();
                    if (units != null && units.Count > 0)
                    {
                        List<Reference<Unit>> list = null;
                        for (int num8 = units.Count - 1; num8 >= 0; num8--)
                        {
                            Reference<Unit> reference = units[num8];
        ...
        private IEnumerator PlayerTurn()
        {
            ...
            foreach (Location item in GameManager.GetLocationsOfWizard(w.GetID()))
            {
        ...
        {
            List<Group> groupsOfPlane = GameManager.GetGroupsOfPlane(p);
            for (int i = 0; i < groupsOfPlane.Count; i++)
            {
                Group group = groupsOfPlane[i];
                if (!group.alive)
                {
                    continue;
                }
                ...
                if ((hexAt.IsLand() && !group.landMovement) || (!hexAt.IsLand() && !group.waterMovement))
                {
                    ...
                    {
                        Unit unit = group.GetUnits()[num].Get();
        ...
        private void EndTurnEffects()
        {
            ...
            {
                Location location = registeredLocations[num];
                if (location.owner > 0)
                {
