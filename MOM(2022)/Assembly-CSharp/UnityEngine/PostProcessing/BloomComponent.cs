namespace UnityEngine.PostProcessing
{
    ...
    public sealed class BloomComponent : PostProcessingComponentRenderTexture<BloomModel>
    {
        ...
        public void Prepare(RenderTexture source, Material uberMaterial, Texture autoExposure)
        {
            ...
            mat.SetFloat(Uniforms._PrefilterOffs, bloom.antiFlicker ? -0.5f : 0f);
            ...
            for (int i = 0; i < num5; i++)
            {
                ...
                Graphics.Blit(texture2, this.m_BlurBuffer1[i], mat, (i == 0) ? 1 : 2);
                texture2 = this.m_BlurBuffer1[i];
            }
            for (int j = num5 - 2; j >= 0; j--)
            {
                RenderTexture texture4 = this.m_BlurBuffer1[j];
                mat.SetTexture(Uniforms._BaseTex, texture4);
                this.m_BlurBuffer2[j] = base.context.renderTextureFactory.Get(texture4.width, texture4.height, 0, format, RenderTextureReadWrite.Default, FilterMode.Bilinear, TextureWrapMode.Clamp, "FactoryTempTexture");
                Graphics.Blit(texture2, this.m_BlurBuffer2[j], mat, 3);
                texture2 = this.m_BlurBuffer2[j];
            }
            RenderTexture texture3 = texture2;
            for (int k = 0; k < 0x10; k++)
            {
                ...
                if ((this.m_BlurBuffer2[k] != null) && (this.m_BlurBuffer2[k] != texture3))
                {
            ...
            uberMaterial.SetTexture(Uniforms._BloomTex, texture3);
        ...
        public override bool active
        {
            get
            {
                return (base.model.enabled && ((base.model.settings.bloom.intensity > 0f) && !base.context.interrupted));
            }
        }
