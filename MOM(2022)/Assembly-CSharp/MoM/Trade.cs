...
namespace MOM
{
    public class Trade : ScreenBase
    {
        ...
        private void Item(GameObject itemSource, object source, object data, int index)
        {
            ...
            else if (source is Artefact)
            {
                Artefact artefact = source as Artefact;
        ...
        private static int GetValue(object o)
        {
            ...
            else if (o is Artefact)
            {
                result = (o as Artefact).GetValue();
            }
            return result;
        }
        ...
        private void UpdateScreen()
        {
            ...
            {
                List<object> items = this.playerWares.FindAll((object o) => o is Artefact);
                this.gridPlayerInventory.UpdateGrid(items, this.gridPlayerInventory);
                items = this.traderWares.FindAll((object o) => o is Artefact);
                this.gridNPCInventory.UpdateGrid(items, this.gridNPCInventory);
            }
            else if (this.tgFilterUnits.isOn)
            {
                List<object> items2 = this.playerWares.FindAll((object o) => o is Unit);
                this.gridPlayerInventory.UpdateGrid(items2, this.gridPlayerInventory);
                items2 = this.traderWares.FindAll((object o) => o is Unit);
        ...
        public void SetData(int playerID, int traderID, NodeTrade.TradeCurrency currency, List<object> playerWares = null, List<object> traderWares = null, int playerParameter = 0, int traderParameter = 0)
        {
            ...
            foreach (object playerWare in this.playerWares)
            {
                ...
                else if (playerWare is Artefact)
                {
                    this.tgFilterItems.gameObject.SetActive(value: true);
                }
                else if (playerWare is Unit)
                {
            ...
            foreach (object traderWare in this.traderWares)
            {
                ...
                else if (traderWare is Artefact)
                {
                    this.tgFilterItems.gameObject.SetActive(value: true);
                }
                else if (traderWare is Unit)
                {
                    this.tgFilterUnits.gameObject.SetActive(value: true);
                }
        ...
        private void ApplyTrade(PlayerWizard wizard, List<object> gain, List<object> loss)
        {
            foreach (object item in loss)
            {
                ...
                if (item is Artefact)
                {
                    if (wizard.artefacts.Contains(item as Artefact))
                    {
                        wizard.artefacts.Remove(item as Artefact);
                        continue;
                    }
                    List<Group> groupsOfWizard = GameManager.GetGroupsOfWizard(wizard.ID);
                    List<Reference<Unit>> list = new List<Reference<Unit>>();
                    foreach (Group item2 in groupsOfWizard)
                    {
                        list.AddRange(item2.GetUnits().FindAll((Reference<Unit> o) => o.Get().dbSource.Get() is Hero));
                    }
                    foreach (Reference<Unit> item3 in list)
                    {
                        if (item3.Get().artefactManager.equipmentSlots.Find((EquipmentSlot o) => o.item == item as Artefact) != null)
                        {
                            item3.Get().artefactManager.equipmentSlots.Find((EquipmentSlot o) => o.item == item as Artefact).item = null;
                            break;
                        }
                    }
                    continue;
            ...
            foreach (object item4 in gain)
            {
                if (item4 is Unit)
                {
                    Unit unit = item4 as Unit;
                    ((Group)unit.group).AddUnit(unit);
                    continue;
                }
                if (item4 is Artefact)
                {
                    wizard.artefacts.Add(item4 as Artefact);
                    continue;
                }
