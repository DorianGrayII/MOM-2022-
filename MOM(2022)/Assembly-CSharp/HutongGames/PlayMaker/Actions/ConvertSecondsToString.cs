...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class ConvertSecondsToString : FsmStateAction
    {
        ...
        {
            TimeSpan timeSpan = TimeSpan.FromSeconds(this.secondsVariable.Value);
            string text = timeSpan.Milliseconds.ToString("D3").PadLeft(2, '0');
            text = text.Substring(0, 2);
            this.stringVariable.Value = string.Format(this.format.Value, timeSpan.Days, timeSpan.Hours, timeSpan.Minutes, timeSpan.Seconds, timeSpan.Milliseconds, timeSpan.TotalDays, timeSpan.TotalHours, timeSpan.TotalMinutes, timeSpan.TotalSeconds, timeSpan.TotalMilliseconds, text);
        }
    }
}
