...
namespace WorldCode
{
    public struct PathNode
    {
        ...
        public void EnsureInitialized(RequestDataV2 rd)
        {
            ...
            {
                this.hCost = new FInt(0.1f * (float)HexCoordinates.HexDistance(this.pos, rd.to));
                if (rd.data.wrapping && this.hCost * 10 > rd.data.width / 2)
                {
                    this.hCost = new FInt(0.1f * (float)rd.data.width) - this.hCost;
                }
            }
            this.sumCost = FInt.ONE * 10000;
            this.cameFrom = -1;
            this.stepCount = 0;
            this.nextItem = -1;
            this.prevItem = -1;
            ...
            this.totalDanger = ((rd.dangerMap != null) ? 1000000 : 0);
        }
