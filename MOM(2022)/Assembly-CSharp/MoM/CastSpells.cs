...
public class CastSpells : SpellBookScreen
{
    ...
    protected List<DBReference<Spell>> GetUnfilteredSpells()
    {
        ...
        if (spellCaster is BattleUnit || spellCaster is global::MOM.Unit)
        {
            bool flag = false;
            ...
            if (spellCaster is global::MOM.Unit unit && unit.dbSource.Get() is Hero)
            {
                flag = true;
            }
    ...
    protected override void UpdateMainButtonState()
    {
        ...
        if (this.WorldMode())
        {
            if (spellCaster is global::MOM.Unit || spellCaster is BattleUnit)
            {
