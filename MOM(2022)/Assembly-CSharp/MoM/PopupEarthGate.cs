...
namespace MOM
{
    public class PopupEarthGate : ScreenBase
    {
        ...
        {
            List<Location> locationsOfWizard = GameManager.GetLocationsOfWizard(this.startTown.GetOwnerID());
            locationsOfWizard = locationsOfWizard.FindAll((Location o) => o is TownLocation && (o as TownLocation).HaveBuilding((Building)BUILDING.EARTH_GATE) && o.GetPlane() == this.startTown.GetPlane());
        ...
        private void TransferUnits()
        {
            ...
            List<Unit> selectedUnits = TownScreen.Get().GetSelectedUnits();
            ...
            else
            {
                ...
                {
                    foreach (Reference<Unit> unit in this.startTown.GetLocalGroup().GetUnits())
                    {
