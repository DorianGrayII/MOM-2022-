...
public class HUD : ScreenBase
{
    ...
    protected override void Awake()
    {
        ...
        MHEventSystem.RegisterListener<global::MOM.Group>(GroupMoved, this);
    ...
    public void ConsiderTutorials()
    {
        if ((bool)this.tutorialNoDefenders && this.tutorialNoDefenders.WouldShow())
        {
            ...
            foreach (global::MOM.Location item in registeredLocations)
            {
    ...
    public override IEnumerator PreStart()
    {
        this.armyGrid.onSelectionChange = delegate(ArmyGrid grid)
        {
            ...
            {
                selectedUnits.Add(selected as global::MOM.Unit);
            }
    ...
    public void UpdateSelectedUnit()
    {
        ...
        if (selectedGroup is global::MOM.Group && (selectedGroup as global::MOM.Group).GetOwnerID() == PlayerWizard.HumanID())
        {
        ...
        this.armyGrid.SetUnits(group?.GetUnits()?.FindAll((Reference<global::MOM.Unit> o) => o.Get().GetWizardOwnerID() == PlayerWizard.HumanID() || !o.Get().IsInvisibleUnit()));
        int num = 0;
    ...
    {
        if (!(FSMSelectionManager.Get().GetSelectedGroup() is global::MOM.Group group) || group.GetUnits().Count() == 0)
        {
            this.ControlButtonsOff();
            return;
        }
        ...
        this.tgGuard.SetIsOnWithoutNotify(group.Action == global::MOM.Group.GroupActions.Guard);
        this.tgSkip.SetIsOnWithoutNotify(group.Action == global::MOM.Group.GroupActions.Skip);
        global::MOM.Location locationAt = GameManager.Get().GetLocationAt(group.GetPosition(), group.GetPlane());
        ...
        {
            global::MOM.Location locationAt2 = GameManager.Get().GetLocationAt(group.GetPosition(), World.GetOtherPlane(group.GetPlane()));
            if (locationAt2 != null && locationAt2 is TownLocation && locationAt2.GetOwnerID() == group.GetOwnerID() && (locationAt2.GetEnchantments().Find((EnchantmentInstance o) => o.source == (Enchantment)ENCH.ASTRAL_GATE) != null || group.GetUnits().Exists((Reference<global::MOM.Unit> u) => u.Get().GetAttributes().Contains(TAG.PLANE_SHIFTER))))
            {
                this.btPlaneTravel.interactable = true;
            }
            else
        ...
        else if (group.GetUnits().Exists((Reference<global::MOM.Unit> u) => u.Get().GetAttributes().Contains(TAG.PLANE_SHIFTER)))
        {
            this.btPlaneTravel.interactable = true;
        }
        else
    ...
    {
        IPlanePosition selectedGroup = FSMSelectionManager.Get().GetSelectedGroup();
        if (selectedGroup == null || selectedGroup is global::MOM.Location)
        {
            this.armyInfo.SetActive(value: false);
        }
        else if (selectedGroup is global::MOM.Group)
        {
    ...
    protected override void ButtonClick(Selectable s)
    {
        ...
        {
            HUD.SwitchSurveyor();
            global::MOM.Group group = FSMSelectionManager.Get().GetSelectedGroup() as global::MOM.Group;
            global::MOM.Group.GroupActions groupActions = ((s == this.tgGuard) ? global::MOM.Group.GroupActions.Guard : global::MOM.Group.GroupActions.Skip);
            if (groupActions == group.Action)
            {
                group.Action = global::MOM.Group.GroupActions.None;
            }
            else
            {
        ...
        if (s == this.btMove)
        {
            HUD.SwitchSurveyor();
            IPlanePosition selectedGroup = FSMSelectionManager.Get().GetSelectedGroup();
            global::MOM.Group group2 = selectedGroup as global::MOM.Group;
        ...
        else if (s == this.btPlaneTravel)
        {
            ...
            global::MOM.Group gr = selectedGroup2 as global::MOM.Group;
            if (gr == null)
            {
                return;
            }
            Debug.Log("Plane: " + gr.GetPlane().planeSource.dbName + ", position: " + gr.GetPosition().ToString());
            global::MOM.Location location = GameManager.GetLocationsOfThePlane(gr.GetPlane()).Find((global::MOM.Location o) => o.GetPosition() == gr.GetPosition());
            ...
            {
                global::MOM.Location locationAt = GameManager.Get().GetLocationAt(gr.GetPosition(), World.GetOtherPlane(gr.GetPlane()));
                if (locationAt != null && locationAt.GetOwnerID() == gr.GetOwnerID() && (locationAt.GetEnchantments().Find((EnchantmentInstance o) => o.source == (Enchantment)ENCH.ASTRAL_GATE) != null || gr.GetUnits().Exists((Reference<global::MOM.Unit> u) => u.Get().GetAttributes().Contains(TAG.PLANE_SHIFTER))) && gr.IsSwitchPlaneDestinationValid())
                {
                    gr.PlaneSwitch();
                }
                return;
            }
            List<Reference<global::MOM.Unit>> list = gr.GetUnits().FindAll((Reference<global::MOM.Unit> o) => o.Get().GetAttributes().Contains(TAG.PLANE_SHIFTER));
            ...
            list.ForEach(delegate(Reference<global::MOM.Unit> o)
            {
                i.Add(o.Get());
            });
        ...
        {
            HUD.SwitchSurveyor();
            if (FSMSelectionManager.Get().GetSelectedGroup() is global::MOM.Group g)
            {
    ...
    {
        if (sender is global::MOM.Group group && group.GetOwnerID() == PlayerWizard.HumanID() && e != null && e is List<Vector3i>)
        {
            this.UpdateEndTurnButtons();
        }
    }
    ...
    {
        foreach (global::MOM.Group item in GameManager.GetGroupsOfWizard(PlayerWizard.HumanID()))
        {
            if (item.alive && item.CurentMP() > 0 && item.Action == global::MOM.Group.GroupActions.None && item.engineerManager == null && item.purificationManager == null)
            {
                return true;
            }
        }
    ...
    public void Update()
    {
        ...
        {
            IPlanePosition selectedGroup = FSMSelectionManager.Get().GetSelectedGroup();
            if (selectedGroup != null && selectedGroup is global::MOM.Group group && group.GetUnits().Find((Reference<global::MOM.Unit> o) => o.Get().IsSettler()) != null)
            {
    ...
    {
        global::MOM.Group curGroup = FSMSelectionManager.Get().GetSelectedGroup() as global::MOM.Group;
        List<global::MOM.Group> list = (from o in GameManager.GetGroupsOfWizard(PlayerWizard.HumanID())
            where o.alive && o.groupUnits.Count > 0
            select o).ToList();
        if (withoutOrders)
        {
            list = list.FindAll((global::MOM.Group o) => !o.isActivelyBuilding && o.CurentMP() > 0 && o.Action == global::MOM.Group.GroupActions.None);
        }
        list.Sort(delegate(global::MOM.Group a, global::MOM.Group b)
        {
        ...
        if (list.Count > 0)
        {
            int num = ((curGroup != null) ? list.FindIndex((global::MOM.Group o) => o == curGroup) : 0);
