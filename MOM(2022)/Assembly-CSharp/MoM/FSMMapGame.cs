...
namespace MOM
{
    ...
    public class FSMMapGame : FSMStateBase
    {
        ...
        {
            if (sender is Formation && (sender as Formation).owner is Group group)
            {
            ...
            while (true)
            {
                ...
                if (!resolved)
                {
                    ...
                    if (enumType == ETargetType.TargetGlobal && (spell == (Spell)SPELL.RESURRECTION || spell == (Spell)SPELL.UNDEAD_HERO))
                    {
                        ...
                        Unit unit = DeadHero.ConvertDeadHeroToUnit(deadHero);
                ...
                if (this.chosenTarget != Vector3i.invalid)
                {
                    ...
                    {
                        Group group2 = GameManager.GetGroupsOfPlane(World.GetActivePlane()).Find((Group o) => o.GetPosition() == this.chosenTarget);
                        if (group2 != null)
                        {
                            ...
                            while (true)
                            {
                                unit2 = MHDataStorage.Get<Unit>("Unit");
                    ...
                    {
                        List<Group> groupsOfPlane = GameManager.GetGroupsOfPlane(World.GetActivePlane());
                        Group group = groupsOfPlane.Find((Group o) => o.GetPosition() == this.chosenTarget);
                        if (group != null)
                        {
                    ...
                    {
                        List<Location> locationsOfThePlane = GameManager.GetLocationsOfThePlane(World.GetActivePlane());
                        Location tar = locationsOfThePlane.Find((Location o) => o.GetPosition() == this.chosenTarget);
                        if (tar == null)
                        {
                            bool flag = false;
                            ...
                            tar = locationsOfThePlane.Find((Location o) => o.GetPosition() == this.chosenTarget);
                    ...
                    else
                    {
                        if (enumType == ETargetType.TargetHex)
                        {
                            ...
                            foreach (Location item in GameManager.GetLocationsOfThePlane(World.GetActivePlane()))
                            {
                        ...
                        if (enumType == ETargetType.WorldHexBattleGlobal)
                        {
                            if (!string.IsNullOrEmpty(spell.targetingScript))
                            {
                                ...
                                foreach (Location item2 in GameManager.GetLocationsOfThePlane(World.GetActivePlane()))
                                {
        ...
        {
            Reference<Unit> reference = o as Reference<Unit>;
            if (reference == null)
            {
        ...
        public override void OnUpdate()
        {
            ...
            else if (Input.GetMouseButtonUp(0) && !UIManager.IsOverUI() && Battle.Get() == null && FSMSelectionManager.Get().roadBuildingMode)
            {
                ...
                HUD.SwitchSurveyor();
                if (selectedGroup != null && selectedGroup is Group && (selectedGroup as Group).GetOwnerID() == PlayerWizard.HumanID())
                {
            ...
            else if (Input.GetMouseButtonUp(1) && !UIManager.IsOverUI() && Battle.Get() == null)
            {
                ...
                HUD.SwitchSurveyor();
                if (selectedGroup2 != null && selectedGroup2 is Group && (selectedGroup2 as Group).GetOwnerID() == PlayerWizard.HumanID())
                {
                    Group group2 = selectedGroup2 as Group;
                    ...
                    else
                    {
                        ...
                        Debug.Log("Path " + text);
                        Group group3 = selectedGroup2 as Group;
                        ...
                        else
                        {
                            ...
                            {
                                bool flag = true;
                                foreach (Unit item2 in selectedUnits)
                                {
                                    if (item2.Mp <= 0)
                                    {
                                        flag = false;
                                ...
                                if (flag)
                                {
                                    Group group4 = new Group(group3.GetPlane(), group3.GetOwnerID());
                                    group4.Position = group3.GetPosition();
                                    foreach (Unit item3 in new List<Unit>(selectedUnits))
                                    {
            ...
            if (Input.GetMouseButtonUp(0) && !UIManager.IsOverUI() && Battle.Get() == null)
            {
                ...
                HUD.SwitchSurveyor();
                if (selectedGroup3 != null && selectedGroup3.GetPosition() == vector3i2 && selectedGroup3 is Group)
                {
                    Location locationAt = GameManager.Get().GetLocationAt(vector3i2);
                ...
                Group group5 = GameManager.Get().GetGroupAt(vector3i2);
                if (group5 != null && group5.GetOwnerID() != PlayerWizard.HumanID())
                {
                    Location locationAt2 = GameManager.Get().GetLocationAt(vector3i2);
                    if (locationAt2 != null)
                    {
                        if (!(locationAt2 is TownLocation) && locationAt2.discovered && locationAt2.explored)
                        {
                            List<Reference<Unit>> units = group5.GetUnits();
                            if (units.Count > 0)
                            {
                ...
                {
                    Location locationAt3 = GameManager.Get().GetLocationAt(vector3i2);
        ...
        {
            Group group = new Group(World.GetActivePlane(), 1);
            ...
            {
                group.AddUnit(Unit.CreateFrom(unit));
            }
            Group group2 = new Group(World.GetActivePlane(), 2);
            ...
            if (unit == null)
            {
                ...
                {
                    group2.AddUnit(Unit.CreateFrom(item2));
                }
            }
            else
            {
                group2.AddUnit(Unit.CreateFrom(unit));
            }
        ...
        public void InitializeRandomBattle(Subrace unit = null, int oponentID = 0, bool ownCapitolBattle = false)
        {
            ...
            if (ownCapitolBattle)
            {
                ...
                List<Reference<Unit>> units = townLocation.GetUnits();
                if (units.Count > 0)
                {
                    group = units[0].Get().group;
                }
                if (FSMSelectionManager.Get() != null && FSMSelectionManager.Get().GetSelectedGroup() is Group group2 && group2.beforeMovingAway == townLocation)
                {
                    group = group2;
                }
            }
            ...
            Group group3 = new Group(World.GetActivePlane(), 2);
            World.GetActivePlane();
            if (unit == null)
            {
                foreach (Subrace item2 in ScriptLibrary.Call("GeneralGroup", 2500) as List<Subrace>)
                {
                    group3.AddUnit(Unit.CreateFrom(item2));
                }
            }
            else
            {
                group3.AddUnit(Unit.CreateFrom(unit));
            }
        ...
        public void InitializeRandomBattle(Subrace unit1, int numUnit1, Subrace unit2, int numUnit2, bool waterBattle = false)
        {
            ...
            {
                group.AddUnit(Unit.CreateFrom(unit1));
            }
            else
            {
            ...
            {
                group2.AddUnit(Unit.CreateFrom(unit2));
            }
            else
            {
        ...
        public void InitializeTestBattle()
        {
            ...
            group.AddUnit(Unit.CreateFrom(source2));
            for (int i = 0; i < 2; i++)
            {
                group.AddUnit(Unit.CreateFrom(source3));
            }
            Group group2 = new Group(World.GetActivePlane(), 0);
            for (int j = 0; j < 2; j++)
            {
                group2.AddUnit(Unit.CreateFrom(source));
            }
            Battle battle = new Battle(group.GetUnits(), group2.GetUnits(), PlayerWizard.HumanID(), 0);
            battle.DebugMode(value: true);
            battle.landBattle = true;
        ...
        public bool PickKeepOrRaze(int attackerID, TownLocation town, Group group)
        {
            if (town.IsAnOutpost())
            {
            ...
            else
            {
                if (attackerID == 0 && (group == null || group.aiNeturalExpedition == null || group.aiNeturalExpedition.rampage))
                {
                    if (global::UnityEngine.Random.Range(0f, 1f) < 0.5f)
                    {
                        ...
                        Location location = Location.CreateLocation(position, plane, type[index], 0, skipMarker: true, spawnDefenders: false);
        ...
        public bool PickSumon(PlayerWizard w, Spell spell, List<Reference<Unit>> prevUnits)
        {
            List<Hero> list = new List<Hero>(DataBase.GetType<Hero>());
            if (spell == (Spell)SPELL.SUMMON_HERO)
            {
                list = list.FindAll((Hero o) => !o.champion && !Unit.HeroInUseByWizard(o, w.GetID()) && o.GetTag(TAG.EVENT_ONLY_UNIT) == 0);
            }
            else if (spell == (Spell)SPELL.SUMMON_CHAMPION)
            {
                list = list.FindAll((Hero o) => o.champion && !o.unresurrectable && !Unit.HeroInUseByWizard(o, w.GetID()) && o.GetTag(TAG.EVENT_ONLY_UNIT) == 0);
            ...
            if (list.Count > 0)
            {
                ...
                while (num < w.GetOfferHeroCount())
                {
                    ...
                    heroUnit = list[num2];
                    if (!Unit.HeroInUseByWizard(heroUnit, w.GetID()) && list2.Find((UnitOffer o) => o.unit.dbSource.Get() == heroUnit) == null)
                    {
                        Unit unit = Unit.CreateFrom(heroUnit);
        ...
        private IEnumerator PickAUnitToSummon(List<UnitOffer> unitOffer, PlayerWizard w, Spell spell, List<Reference<Unit>> prevUnits)
        {
            Unit picked = null;
            if (w.IsHuman)
            {
            ...
            List<Reference<Unit>> units = w.summoningCircle.Get().GetUnits();
            Unit unit2 = null;
            foreach (Reference<Unit> item3 in units)
            {
