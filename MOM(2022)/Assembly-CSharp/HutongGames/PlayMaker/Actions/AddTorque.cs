namespace HutongGames.PlayMaker.Actions
{
    ...
    {
        [RequiredField, CheckForComponent(typeof(Rigidbody)), HutongGames.PlayMaker.Tooltip("The GameObject to add torque to.")]
        public FsmOwnerDefault gameObject;
        [UIHint(UIHint.Variable), HutongGames.PlayMaker.Tooltip("A Vector3 torque. Optionally override any axis with the X, Y, Z parameters.")]
        ...
        private void DoAddTorque()
        {
            ...
            {
                Vector3 torque = this.vector.IsNone ? new Vector3(this.x.Value, this.y.Value, this.z.Value) : this.vector.get_Value();
                if (!this.x.IsNone)
                {
