namespace HutongGames.PlayMaker.Actions
{
    ...
    public class AxisEvent : FsmStateAction
    {
        ...
        {
            float x = (this.horizontalAxis.Value != "") ? Input.GetAxis(this.horizontalAxis.Value) : 0f;
            float y = (this.verticalAxis.Value != "") ? Input.GetAxis(this.verticalAxis.Value) : 0f;
            if (((x * x) + (y * y)).Equals((float) 0f))
            {
