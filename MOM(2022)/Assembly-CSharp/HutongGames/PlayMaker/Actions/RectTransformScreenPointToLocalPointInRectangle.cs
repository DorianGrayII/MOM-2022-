...
[global::HutongGames.PlayMaker.Tooltip("Transform a screen space point to a local position that is on the plane of the given RectTransform. Also check if the plane of the RectTransform is hit, regardless of whether the point is inside the rectangle.")]
public class RectTransformScreenPointToLocalPointInRectangle : FsmStateAction
{
    ...
    [global::HutongGames.PlayMaker.Tooltip("Store the local Position as a vector3 of the screenPoint on the RectTransform Plane.")]
    [UIHint(UIHint.Variable)]
    public FsmVector3 localPosition;

    [global::HutongGames.PlayMaker.Tooltip("Store the local Position as a vector2 of the screenPoint on the RectTransform Plane.")]
    [UIHint(UIHint.Variable)]
    ...
    private void DoCheck()
    {
        ...
        bool flag = false;
        flag = RectTransformUtility.ScreenPointToLocalPointInRectangle(this._rt, value, this._camera, out var localPoint);
        if (!this.localPosition2d.IsNone)
        {
            this.localPosition2d.Value = localPoint;
        }
        if (!this.localPosition.IsNone)
        {
            this.localPosition.Value = new Vector3(localPoint.x, localPoint.y, 0f);
        }
