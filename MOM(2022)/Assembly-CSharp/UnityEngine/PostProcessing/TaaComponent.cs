namespace UnityEngine.PostProcessing
{
    ...
    public sealed class TaaComponent : PostProcessingComponentRenderTexture<AntialiasingModel>
    {
        ...
        private Vector2 GenerateRandomOffset()
        {
            ...
            return new Vector2(this.GetHaltonValue(this.m_SampleIndex & 0x3ff, 2), this.GetHaltonValue(this.m_SampleIndex & 0x3ff, 3));
        }
        ...
        private unsafe Matrix4x4 GetOrthographicProjectionMatrix(Vector2 offset)
        {
            ...
            float* singlePtr1 = &offset.x;
            singlePtr1[0] *= num2 / (0.5f * base.context.width);
            float* singlePtr2 = &offset.y;
            singlePtr2[0] *= orthographicSize / (0.5f * base.context.height);
        ...
        private unsafe Matrix4x4 GetPerspectiveProjectionMatrix(Vector2 offset)
        {
            ...
            singlePtr1[0] *= num2 / (0.5f * base.context.width);
            float* singlePtr2 = &offset.y;
            singlePtr2[0] *= num / (0.5f * base.context.height);
        ...
        public void Render(RenderTexture source, RenderTexture destination)
        {
            ...
            GraphicsUtils.Blit(mat, base.context.camera.orthographic ? 1 : 0);
            RenderTexture.ReleaseTemporary(this.m_HistoryTexture);
        ...
        {
            Vector2 offset = this.GenerateRandomOffset() * base.model.settings.taaSettings.jitterSpread;
            base.context.camera.nonJitteredProjectionMatrix = base.context.camera.projectionMatrix;
            base.context.camera.projectionMatrix = (jitteredFunc == null) ? (base.context.camera.orthographic ? this.GetOrthographicProjectionMatrix(offset) : this.GetPerspectiveProjectionMatrix(offset)) : jitteredFunc(offset);
            base.context.camera.useJitteredProjectionMatrixForTransparentRendering = false;
            float* singlePtr1 = &offset.x;
            singlePtr1[0] /= (float) base.context.width;
            float* singlePtr2 = &offset.y;
            singlePtr2[0] /= (float) base.context.height;
        ...
        public override bool active
        {
            get
            {
                return (base.model.enabled && ((base.model.settings.method == AntialiasingModel.Method.Taa) && (SystemInfo.supportsMotionVectors && ((SystemInfo.supportedRenderTargetCount >= 2) && !base.context.interrupted))));
            }
        }
