...
namespace MOM
{
    ...
    public class GameManager : Entity, IEnchantable
    {
        ...
        [ProtoIgnore]
        public Dictionary<global::WorldCode.Plane, List<Group>> groups = new Dictionary<global::WorldCode.Plane, List<Group>>();

        [ProtoIgnore]
        public Dictionary<global::WorldCode.Plane, List<Location>> locations = new Dictionary<global::WorldCode.Plane, List<Location>>();
        ...
        [ProtoIgnore]
        public List<Location> registeredLocations = new List<Location>();
        ...
        public void PostLoad()
        {
            ...
            this.registeredGroups = EntityManager.GetEntitiesType<Group>();
            if (this.registeredGroups != null)
            {
                this.groups.Clear();
                foreach (Group registeredGroup in this.registeredGroups)
                {
                    global::WorldCode.Plane plane = registeredGroup.GetPlane();
                    if (!this.groups.ContainsKey(plane))
                    {
                        this.groups[plane] = new List<Group>();
                    }
                    this.groups[plane].Add(registeredGroup);
                    Location locationHost = registeredGroup.GetLocationHost();
            ...
            this.registeredLocations = EntityManager.GetEntitiesType<Location>();
            if (this.registeredLocations != null)
            {
                this.locations.Clear();
                foreach (Location registeredLocation in this.registeredLocations)
                {
                    global::WorldCode.Plane plane2 = registeredLocation.GetPlane();
                    if (!this.locations.ContainsKey(plane2))
                    {
                        this.locations[plane2] = new List<Location>();
                    }
        ...
        public void RegisterGroup(Group g)
        {
            ...
            {
                this.groups[g.GetPlane()] = new List<Group>();
            }
            g.alive = true;
        ...
        public void RegisterLocation(Location l)
        {
            ...
            {
                this.locations[l.plane] = new List<Location>();
            }
        ...
        public void Unregister(Group g)
        {
            if (g == null)
            {
                return;
            ...
            if (g.GetUnits() != null)
            {
                foreach (Reference<Unit> unit in g.GetUnits())
                {
        ...
        public void Unregister(Location l)
        {
        ...
        public Group GetGroupAt(Vector3i pos, global::WorldCode.Plane p = null)
        {
            if (p == null)
            {
            ...
            {
                return this.groups[p].Find((Group o) => o.GetPosition() == pos);
            }
            return null;
        }

        public static List<Group> GetGroupsOfPlane(bool arcanus)
        {
            if (arcanus)
            {
        ...
        public static List<Group> GetGroupsOfPlane(global::WorldCode.Plane p)
        {
        ...
        public Location GetLocationAt(Vector3i pos, global::WorldCode.Plane p = null)
        {
            if (p == null)
            {
                p = World.GetActivePlane();
            ...
            {
                return this.locations[p].Find((Location o) => o.Position == pos);
            }
            return null;
        }

        public static List<Location> GetLocationsOfThePlane(bool arcanus)
        {
            if (arcanus)
            {
        ...
        public static List<Location> GetLocationsOfThePlane(global::WorldCode.Plane p)
        {
            if (p == null)
            {
                p = World.GetActivePlane();
        ...
        public static List<Location> GetLocationsOfWizard(int ID)
        {
        ...
        public static List<Location> GetTownsOfWizard(int ID)
        {
            List<Location> list = new List<Location>();
            foreach (Location registeredLocation in GameManager.Get().registeredLocations)
            {
        ...
        public static List<Group> GetGroupsOfWizard(int ID)
        {
        ...
        {
            List<Group> groupsOfWizard = GameManager.GetGroupsOfWizard(PlayerWizard.HumanID());
            int value = TurnManager.GetTurnNumber() * 3 + 100;
            foreach (Group item in groupsOfWizard)
            {
        ...
        public static void InitializePathfindingObstacles()
        {
            foreach (global::WorldCode.Plane plane in World.GetPlanes())
            {
                ...
                foreach (Group item in GameManager.GetGroupsOfPlane(plane))
                {
                    GameManager.SetPathfindingObstacleAt(plane, item.GetPosition(), value: true);
                }
                foreach (Location item2 in GameManager.GetLocationsOfThePlane(plane))
                {
        ...
        {
            bool flag = false;
            foreach (Group item in GameManager.GetGroupsOfPlane(p))
            {
            ...
            if (!flag)
            {
                foreach (Location item2 in GameManager.GetLocationsOfThePlane(p))
                {
        ...
        public static bool GroupAlive(IPlanePosition e)
        {
            if (e is Group)
            {
                return GameManager.Get().registeredGroups.Contains(e as Group);
            }
            if (e is Location)
            {
                return GameManager.Get().registeredLocations.Contains(e as Location);
            }
        ...
        public void AllowPlaneSwitch(bool value)
        {
            ...
            if (!value)
            {
                foreach (Location item in GameManager.GetLocationsOfThePlane(arcanus: true))
                {
