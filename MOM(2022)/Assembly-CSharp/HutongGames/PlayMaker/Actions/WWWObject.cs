namespace HutongGames.PlayMaker.Actions
{
    ...
    [ActionCategory("WWW"), Tooltip("Gets data from a url and store it in variables. See Unity WWW docs for more details.")]
    public class WWWObject : FsmStateAction
    {
        ...
        [UIHint(UIHint.Variable), Tooltip("Gets a Texture from the url.")]
        ...
        [UIHint(UIHint.Variable), Tooltip("How far the download progressed (0-1).")]
        public FsmFloat progress;
        [ActionSection("Events"), Tooltip("Event to send when the data has finished loading (progress = 1).")]
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (this.progress.Value.Equals((float) 1f))
                    {
                        ...
                        if (!this.storeTexture.IsNone)
                        {
                            this.storeTexture.set_Value(((DownloadHandlerTexture) this.uwr.downloadHandler).texture);
                        }
                        this.errorString.Value = this.uwr.error;
