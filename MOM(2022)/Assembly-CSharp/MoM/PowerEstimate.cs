...
public class PowerEstimate
{
    ...
    public static void LoadData()
    {
        ...
        {
            global::MOM.Unit unit = global::MOM.Unit.CreateFrom(item, simulation: true);
    ...
    public static IEnumerator SimulatedBattle(Battle source, int iterations, BattleResult returnResult, int pickBest = 0, bool aUseMana = true, bool dUseMana = true)
    {
        ...
        if (source != null)
        {
            ...
            foreach (KeyValuePair<BattleUnit, global::MOM.Unit> item in source.buToSource)
            {
    ...
    private static void TestSpells(BattlePlayer aPlayer, BattlePlayer dPlayer)
    {
        ...
        global::MOM.Group group = new global::MOM.Group(World.GetArcanus(), aPlayer.GetID());
        List<BattleUnit> list = new List<BattleUnit>();
        global::MOM.Group group2 = new global::MOM.Group(World.GetArcanus(), dPlayer.GetID());
        List<BattleUnit> list2 = new List<BattleUnit>();
        foreach (Subrace item in obj)
        {
            group.AddUnit(global::MOM.Unit.CreateFrom(item));
            group2.AddUnit(global::MOM.Unit.CreateFrom(item));
        }
        foreach (Reference<global::MOM.Unit> unit in group.GetUnits())
        {
            list.Add(BattleUnit.Create(unit));
        }
        foreach (Reference<global::MOM.Unit> unit2 in group2.GetUnits())
        {
