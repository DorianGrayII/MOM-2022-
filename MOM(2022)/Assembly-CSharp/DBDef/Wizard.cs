namespace DBDef
{
    ...
    public class Wizard : DBClass, IDescriptionInfoType
    {
        ...
        {
            return DataBase.Get<Wizard>(e, false);
        }

        public static explicit operator Wizard(string e)
        {
            return DataBase.Get<Wizard>(e, true);
        }
            ...
            if ((list != null) && (list.Count != 0))
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    if (!(list[i] is Associate_Greeting))
                    {
                        ...
                        Debug.LogError("associateGreeting of type Associate_Greeting received invalid type from array! " + text1);
                    }
            ...
            if ((list != null) && (list.Count != 0))
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    if (!(list[i] is Associate_Greeting))
                    {
                        ...
                        Debug.LogError("associateInitialGreeting of type Associate_Greeting received invalid type from array! " + text1);
                    }
            ...
            if ((list != null) && (list.Count != 0))
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    if (!(list[i] is Trait))
                    {
                        ...
                        Debug.LogError("traits of type Trait received invalid type from array! " + text1);
                    }
