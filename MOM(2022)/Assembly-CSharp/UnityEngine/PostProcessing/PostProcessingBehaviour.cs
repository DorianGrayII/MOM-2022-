namespace UnityEngine.PostProcessing
{
    ...
    public class PostProcessingBehaviour : MonoBehaviour
    {
        ...
        {
            CommandBuffer buffer1 = new CommandBuffer();
            buffer1.name = name;
            CommandBuffer buffer = buffer1;
        ...
        private void OnPreCull()
        {
            ...
            if ((this.profile != null) && (this.m_Camera != null))
            {
                ...
                foreach (PostProcessingComponentBase base2 in this.m_Components)
                {
                    if (base2.active)
                    {
                        depthTextureMode |= base2.GetCameraFlags();
                    }
                }
        ...
        private void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            else
            {
                ...
                if (antialiasCoC)
                {
                    RenderTexture texture4 = this.m_RenderTextureFactory.Get(baseRenderTexture);
                    this.m_Taa.Render(baseRenderTexture, texture4);
                    baseRenderTexture = texture4;
                }
                ...
                if (this.m_DepthOfField.active && !this.m_RenderingInSceneView)
                {
                    flag = true;
                ...
                Material material = active ? this.m_MaterialFactory.Get("Hidden/Post FX/FXAA") : null;
                if (!active)
                {
